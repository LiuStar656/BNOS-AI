"""
BNOS Python 节点监听器 — 多源监听 + 线程池并发 + venv 自愈（node_dsh 版）
...
# 兜底超时：须 > main.py 内部 DSH_TIMEOUT(600)。main.py 超时后先整树杀
# DSH 子进程再写回"任务超时"结果；listener 的 timeout 只在节点自身失联时兜底，
# 避免竞态（listener 先杀 main.py → node.exe 成孤儿永久残留）。
...
def _process_one(src_path: str, raw_data: dict):
    ...
    try:
        ...
        if _is_frozen_exe():
            ...
            if res.returncode != 0:
                log(f"main.py 返回非零: {res.stderr[:300]}", "ERROR")
                return
