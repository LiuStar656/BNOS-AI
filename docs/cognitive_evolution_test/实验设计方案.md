...
| directness 维度死寂 | 三组 directness 始终 ~0.5，零漂移（warmth/playfulness 正常演化） | 中 |
...
| E5 | 多后端交叉验证 | 认知系统的结构性行为（饱和/directness 死寂）不依赖特定 LLM | 100 | 6 | P1 |
| E6 | 命令污染治理 | 命令句式检测 + 频次门槛可将污染从 47 降至 < 10 | 100 | 4 | P0 |
| E7 | directness 维度排查 | directness 死寂根因是风格估计盲区或演化逻辑缺失 | 100 | 3 | P1 |
...
1. **directness 维度死寂**：三组 directness 始终 ~0.5，warmth 和 playfulness 正常演化，directness 零漂移
...
| 组 | 输入策略 | 初始种子 | 轮次 | 目的 |
|----|----------|----------|:----:|------|
| E2-A | 全正面（持续鼓励） | 默认 [0.6,0.4,0.5,0.5] | 200 | 正向情绪驱动的演化方向 + directness 是否仍死寂 |
| E2-B | 正负交替（50 正→50 负→50 正→50 负） | 默认 [0.6,0.4,0.5,0.5] | 200 | 情绪震荡下的向量波动 + directness 交叉验证 |
| E2-C | 全负面（极端压力） | 温柔型 [0.8,0.5,0.3,0.6] | 200 | 不同种子对相同压力的响应差异 + directness=0.3 起点是否仍死寂 |

### directness 死寂诊断

增强实验中三组 directness 演化数据：

| 组 | directness 初始 | directness 终值 | 变化量 |
|----|:--------------:|:--------------:|:------:|
| main | 0.5 | 0.5 | 0.000 |
| controlA | 0.5 | 0.503 | 0.003 |
| controlB | 0.5 | 0.513 | 0.013 |

E2-C 使用 directness=0.3 的温柔型种子，若仍死寂则排除"中值盲区"假设（即 0.5 附近无梯度），确认是风格估计或演化逻辑缺陷。
...
- E2-A：warmth 维度应上升，directness 可能仍死寂（若 E7 修复未合并）
- E2-B：向量在正负交替下反复震荡，净漂移接近 0，directness 仍不动
- E2-C：温柔型种子在负面压力下 warmth 下降更快；directness=0.3 起点若仍不动 → 确认风格估计盲区（联动 E7）
...
增强实验已验证向量可以漂移，但 directness 维度死寂（见 E7）。本实验同时观察 directness 在不同种子起点下的行为。
...
> **注意**：毒舌型 directness=0.9、温柔型 directness=0.3，若三组 directness 仍不动，则与 E2-C（0.3 起点不动）共同确认 directness 死寂是系统性缺陷。
...
- directness：若三种种子的 directness 均不动 → 确认是系统性缺陷（联动 E7）
...
> **变更说明**：增强实验已证明性格向量可以漂移（不再是"保持稳定"），因此原假设"所有后端向量均保持稳定"已过时。更新为：认知系统的**结构性行为模式**（情绪饱和、directness 死寂、命令污染模式）不依赖于特定 LLM 后端。

认知系统的结构性行为是系统架构决定的，不依赖于特定 LLM 后端。不同 LLM 会影响内容质量、演化幅度和速度，但不会改变结构性结论（饱和会发生、directness 会死寂、命令会产生污染）。
...
| directness 演化轨迹 | 逐轮 directness 值（跨模型对比是否均死寂） |
| 自我认知条数 | COUNT(*) |
...
| directness 死寂 | 所有后端 directness 变化 < 0.02 | 0.000-0.013 |
...
- 所有后端的 directness 维度均死寂（结构性缺陷，非模型差异）
...
## E7：directness 维度排查实验

### 假设

增强实验发现 directness 维度在所有三组中死寂（变化量 0.000-0.013），而 warmth 和 playfulness 正常演化。可能根因：
1. **风格估计盲区**：`estimate_style_from_reply` 对 directness 的词典命中率为零或极低
2. **演化逻辑缺失**：`_adjust_vector` 对 directness 维度无梯度或步长为零
3. **观测-反馈断路**：LLM 回复的 directness 风格确实在变化，但估计函数无法捕获

### 设计

| 组 | 策略 | 轮次 | 目的 |
|----|------|:----:|------|
| E7-A | 诊断模式：记录每轮 `estimate_style_from_reply` 四维原始输出 | 100 | 定位死寂层：估计盲区 or 演化逻辑 |
| E7-B | 修补估计：扩充 directness 词典 + 语法特征检测 | 100 | 验证修补后 directness 是否开始漂移 |
| E7-C | 极端 directness 输入：使用极高/极低 directness 对话池 | 100 | 验证极端输入能否触发 directness 演化 |
...
def diagnose_directness(reply_text, estimated_style):
    ...
    diagnosis = {
        "reply_text": reply_text[:200],  # 截取前 200 字
        "estimated_style": estimated_style,  # 四维估计值
        "directness_value": estimated_style.get("directness", None),
        # 词典命中详情
        "directness_high_hits": [],  # 命中的高 directness 词
        "directness_low_hits": [],   # 命中的低 directness 词
        "sentence_patterns": [],     # 检测到的语法特征（祈使句、反问句等）
    }

    DIRECTNESS_HIGH_WORDS = ["直接", "必须", "应该", "毫无疑问", "绝对", "当然", "明确"]
    DIRECTNESS_LOW_WORDS = ["也许", "可能", "大概", "或许", "我觉得", "不确定", "可能吧"]
    ...
    # 语法特征：祈使句、感叹号密度
    diagnosis["exclamation_ratio"] = reply_text.count("！") / max(len(reply_text), 1)
    diagnosis["imperative_count"] = len(re.findall(r"[。！]", reply_text))
...
```python
def patched_estimate_style(reply_text):
    """扩充版风格估计：增加 directness 语法特征检测"""
    style = orig_estimate_style(reply_text)

    # 语法特征补充
    directness_signals = 0.0
    signal_count = 0

    # 祈使句检测
    if re.search(r"(必须|应该|一定|务必|不准|禁止)", reply_text):
        directness_signals += 0.8
        signal_count += 1

    # 犹豫语气检测
    if re.search(r"(也许|可能|大概|或许|不确定|我觉得可能)", reply_text):
        directness_signals += 0.2
        signal_count += 1

    # 感叹号密度
    excl_ratio = reply_text.count("！") / max(len(reply_text), 1)
    if excl_ratio > 0.02:
        directness_signals += 0.7
        signal_count += 1

    # 句式长度（短句=直接）
    avg_sent_len = len(reply_text) / max(reply_text.count("。") + reply_text.count("！") + 1, 1)
    if avg_sent_len < 15:
        directness_signals += 0.7
        signal_count += 1
    ...
    if signal_count > 0:
        # 融合词典估计和语法估计
        dict_directness = style.get("directness", 0.5)
        grammar_directness = directness_signals / signal_count
        style["directness"] = 0.5 * dict_directness + 0.5 * grammar_directness
...
| directness 语法特征 | E7-A 诊断记录中的 sentence_patterns |
| directness 变化量 | 终值 - 初始值 |
| 其他三维对照 | warmth/playfulness/curiosity 逐轮值（确认非全局死寂） |
...
- E7-A：诊断结果将揭示死寂根因：
  - 若词典命中为零 → 估计盲区确认
  - 若词典命中但 directness 不变 → 演化逻辑缺失确认
  - 若两者均正常但 directness 不变 → 观测-反馈断路确认
- E7-B：修补估计后 directness 应开始漂移（变化量 > 0.02）
- E7-C：极端高 directness 输入 → directness 应上升；极端低 → 应下降。若仍不动则确认演化逻辑缺陷
...
| 4 | E7 | ~1 小时（3 组×100 轮×~3s/轮） | E2 directness 数据 | P1 |
...
| E7 directness 诊断依赖回复文本 | 语法特征可能不够全面 | 结合人工抽样评分校准 |
...
5. directness 死寂根因报告（估计盲区/演化逻辑/断路）
