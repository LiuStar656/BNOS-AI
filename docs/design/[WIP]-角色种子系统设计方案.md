...
给 AI 一个"来历"，避免第一天完全空白。写入 `event_summary` 表，`source='seed'` 标记（背景属于事件而非对话 QA，不写入长期记忆表）：
...
写入现有的 `event_summary` 表，`source='seed'` 标记（初始背景属于事件摘要，非对话 QA）：

```sql
INSERT INTO event_summary (conversation_id, identity_key, summary, source, created_at)
VALUES ('default', ?, '我刚来到这台电脑上，对用户还不了解', 'seed', datetime('now','localtime'));
```

随真实事件摘要积累，初始背景自然淡化；事件摘要同时是记忆图谱的数据源之一。
...
| `db.py` 新增初始背景记忆写入 | 写入 `event_summary`，`source='seed'` | 无 |
...
| 操作 | 说明 |
|------|------|
| 清空记忆 | 删除所有用户数据表中的记录（含 `fixed_cognition` 固定认知，遍历逻辑见 §10.5） |
| 重置性格 | 将 `personality_seed` 表恢复为默认种子值（`warmth=0.6, playfulness=0.4, directness=0.5, curiosity=0.5`） |
| 弹出选择 | 重置后自动弹出性格选择界面，让用户重新选择预设种子 |

> **v2.1 合并说明**：人格格式化与"清空数据库"合并为一个功能。原 `clear` 命令已移除，`format` 即**彻底清空全部表（含 `fixed_cognition`）** + 重置性格 + 重新选择。
...
| `fixed_cognition` 表 | ✗ 清空 | v2.1 已与"清空数据库"合并，格式化即彻底清空 |
...
- **清空所有用户表**：`SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'` 遍历删除（含 `fixed_cognition`，位于 `aaa_cognition/main.py::_on_db_command` 的 `cmd == "format"` 分支）
- **`personality_seed` 特殊处理**：清空后 INSERT 默认值（`_IDENTITY_KEY_DEFAULT`, `warmth=0.6`, `playfulness=0.4`, `directness=0.5`, `curiosity=0.5`）
- **GUI 端**：`settings_panel.py` 提供"人格格式化（清空并重来）"按钮 + 确认弹窗 + 格式化完成后回调弹出种子选择界面
- **后端接口**：`db_command` 类型 `cmd="format"`（v2.1 起 `cmd="clear"` 已移除，二者合并）
- **对话历史**：`conversation_history.json` 也应清空，避免 UI 残留旧对话

#### 10.5.1 格式化 = 清空数据库（v2.1 合并说明）

> v2.1 起「清空数据库」与「人格格式化」合并为一个功能，仅保留 `format` 命令。格式化即**彻底清空所有用户表（含 `fixed_cognition`）**，不再区分保留表。

| 表名 | `format`（人格格式化 = 清空数据库） | 原因 |
|------|:---:|------|
| 对话/记忆/认知/事件摘要/日记/用户画像等用户表 | ✗ 清空 | 核心重置目标 |
| `personality_seed` | 重置为默认值（不删除表，INSERT 默认种子） | 性格一起重置 |
| `mood_value` | ✗ 清空 | 情绪历史重置 |
| `fixed_cognition` | ✗ 清空 | 格式化即彻底清空，无保留 |
| `conversation_history.json` | ✗ 清空 | 避免 UI 残留旧对话 |
| 系统表（`sqlite_*`） | 保留 | SQLite 内部表 |
| `gui_config.json` | 保留 | GUI 偏好与 AI 人格无关 |

**实现方式**：`format` 命令在 `_on_db_command` 中遍历删除全部用户表，仅 `personality_seed` 在清空后写默认种子：

```python
# format 分支伪代码
for (tname,) in tables:              # 所有非 sqlite_ 表，含 fixed_cognition
    conn.execute(f"DELETE FROM [{tname}]")
db.reset_personality_seed(dbp)       # personality_seed 重置为默认种子
self._clear_conversation_history()   # 清空 GUI 对话历史 JSON
# → 回调 GUI 弹出性格选择界面（首次启动流程复用）
```

**历史兼容说明**：v2.0 中 `format` 曾保留 `fixed_cognition`（系统级固定认知），v2.1 依据使用反馈改为彻底清空——AI 的人格格式化不应保留任何旧认知痕迹。

### 10.6 与现有功能的关系

| 现有功能 | 关系 |
|---------|------|
| 清空数据库（`clear`） | **v2.1 已合并移除**：`clear` 命令删除，`format` = 彻底清空全部表 + 重置性格 + 弹出选择 |
...
| 项 | 要求 |
|------|------|
| 操作系统 | Windows 10 / 11（与开发环境一致） |
| Python 版本 | 3.10 及以上（代码使用 `list[dict]`、`tuple[int, int] \| None` 等新语法） |
| 数据库 | `chatbot.db` 可读写，已含现有 11 张表且 `identity_key` 多用户隔离已上线 |
| 新增数据表 | `personality_seed`、`mood_value`（含 `idx_mv_identity`、`idx_mv_time` 索引）已建表 |
| GUI 框架 | PySide6 >= 6.5.0（`requirements.txt` 现有依赖，不额外引入图表库） |
| LLM 推理节点 | `node_python_llm_infer` 可正常调用，能解析 `【情绪调整】` 输出标签 |
| MemOS | `memos.py` 已部署，语义检索 + decay + `identity_key` 隔离可用 |
| ASR / TTS 节点 | 可选；用于验证反馈信号采集（打断 TTS、ASR 情绪检测） |
| 测试工具 | SQLite 可视化工具（查表/查记录）、日志查看工具、可捕获发往 LLM 的完整 prompt |
| 前置数据 | 验收前 `personality_seed`、`mood_value` 表为空或已知状态，便于断言 |

### 11.2 功能验收用例

| 编号 | 验收项 | 操作步骤 | 预期结果 | 通过标准 | 类型 |
|:----:|------|---------|---------|---------|:----:|
| F1 | 数据库表初始化 | 启动系统后用 SQLite 工具检查 `chatbot.db` | `personality_seed` 表（含 8 个字段）与 `mood_value` 表（含 7 个字段 + 2 个索引）均存在，字段类型与 §3.5 设计一致 | 表结构与设计完全一致，CRUD 方法可正常调用 | 核心 |
| F2 | 首次启动预设种子选择 | 清空 `personality_seed` 后首次启动 GUI，在性格选择界面点击"温柔型"预设卡片 | `personality_seed` 表写入一条记录：warmth=0.8、playfulness=0.5、directness=0.3、curiosity=0.6，preset_name='温柔型'，style_description 非空 | 4 维数值与 §3.1.2 预设表一致，风格描述已写入 | 核心 |
| F3 | 自定义滑块微调 | 在性格选择界面或设置面板将 warmth 滑块拖至 0.7 并保存 | `personality_seed` 表 warmth 字段更新为 0.7，其余 3 维不变 | 数值精确持久化，重启后仍为 0.7 | 非核心 |
| F4 | 冷启动第一句话风格差异 | 分别用"温柔型"与"毒舌型"种子完成首次启动，各记录 AI 第一句开场白 | 温柔型开场白语气关心柔和，毒舌型开场白语气直接调侃，两句风格明显不同 | 人工评估两句风格可区分，且符合各自预设关键词 | 核心 |
| F5 | prompt 注入性格与情绪段 | 触发一次 LLM 推理，捕获发往 LLM 的完整 prompt | prompt 含"你的性格"段（4 维数值）与"当前情绪"段（情绪值 + 描述），数值与 DB 当前值一致 | `{personality}` 与 `{mood}` 占位符已被实际值替换，无残留占位符 | 核心 |
| F6 | 情绪调整标签解析与累加 | 记录当前情绪值 X，令 LLM 输出 `【情绪调整】:+0.15`，处理后查 `mood_value` 表 | 新情绪值 = X + 0.15，`mood_value` 新增一条记录且 adjustment=0.15 | 累加计算正确并已持久化 | 核心 |
| F7 | 反馈信号采集 | AI 回复后分两次测试：① 5 秒内继续输入；② 30 秒无任何输入 | ① memory 表写入 source='feedback'、reaction='positive'；② 写入 reaction='negative' | 两种反馈信号被正确分类并记录 | 核心 |
| F8 | 情绪驱动性格演化触发 | 构造连续 10 次情绪平均值 > 0.3 的场景，观察是否触发 `_adjust_vector` | 第 10 次后触发性格微调，相关维度按 ±0.02 × 1.5 权重调整并写回 DB | 触发时机正确，调整幅度符合 §3.3.2 预期 | 核心 |
| F9 | 兜底演化触发 | 构造情绪长期中性（均值在 -0.3 ~ 0.3 之间）场景，累计 30 次交互 | 第 30 次交互触发 `_adjust_vector` | 兜底机制在情绪驱动未触发时生效 | 非核心 |
| F10 | 性格向量边界约束 | 将某维度构造为 0.99，触发一次 +0.02 的正向微调 | 微调后值为 1.0，不超出上限；反向同理不低于 0.0 | 向量始终被 clamp 在 [0.0, 1.0] | 核心 |
| F11 | 多用户隔离 | 用 identity_key='userA' 与 'userB' 分别写入不同性格向量与情绪值，再互相读取 | userA 仅读到自身数据，userB 仅读到自身数据 | 两个 identity_key 数据完全隔离，互不污染 | 核心 |
| F12 | 演化与情绪持久化 | 修改性格向量与情绪值后重启系统，重新从 DB 读取 | 重启后性格向量与情绪值与重启前完全一致 | 数据持久化可靠，无丢失 | 核心 |
| F13 | 情绪曲线可视化 | 在知识库面板切换到"情绪曲线"Tab | `MoodChartWidget` 加载 `mood_value` 数据并绘制折线图，正面区域绿色填充、负面区域红色填充、y=0 基准线显示 | 曲线正确渲染，无额外依赖引入 | 非核心 |
| F14 | 情绪曲线时间范围切换 | 在情绪曲线 Tab 依次点击"最近50次""最近7天""最近30天""全部" | 曲线按所选范围重新加载数据并刷新 | 4 种范围切换均正确刷新 | 非核心 |
| F15 | 人格格式化 | 在设置面板点击"人格格式化（清空并重来）"，完成二次确认 | 所有用户表（含 `fixed_cognition`）被清空，`personality_seed` 重置为默认值，`mood_value` 清空，`conversation_history.json` 清空，自动弹出性格选择界面 | 数据彻底重置（无保留表）且引导流程正常弹出 | 核心 |

### 11.3 边界与异常验收

| 编号 | 验收项 | 操作步骤 | 预期结果 | 通过标准 | 类型 |
|:----:|------|---------|---------|---------|:----:|
| E1 | LLM 情绪调整值超上限 | 令 LLM 输出 `【情绪调整】:+0.5`，处理后查 `mood_value` 表 | 代码层强制限制为 +0.2 后再累加，adjustment 字段记录为 0.2 | 实际生效 adjustment = 0.2，不超限 | 核心 |
| E2 | LLM 情绪调整值超下限 | 令 LLM 输出 `【情绪调整】:-0.5`，处理后查 `mood_value` 表 | 代码层强制限制为 -0.2 后再累加，adjustment 字段记录为 -0.2 | 实际生效 adjustment = -0.2，不超限 | 核心 |
| E3 | 情绪累加上限 clamp | 当前情绪值设为 0.9，LLM 输出 `【情绪调整】:+0.2` | 新情绪值 = 1.0（不超 1.0） | clamp 到 [-1.0, 1.0] 上限生效 | 核心 |
| E4 | 情绪累加下限 clamp | 当前情绪值设为 -0.9，LLM 输出 `【情绪调整】:-0.2` | 新情绪值 = -1.0（不低于 -1.0） | clamp 到 [-1.0, 1.0] 下限生效 | 核心 |
| E5 | 情绪调整值解析失败 | 令 LLM 输出 `【情绪调整】:abc`，观察处理流程 | 解析失败，adjustment 默认为 0.0，情绪值不变，系统不崩溃 | 异常被 try/except 捕获，无报错中断 | 非核心 |
| E6 | 无种子时 fallback | 删除 `personality_seed` 记录后触发一次 prompt 构建 | 返回默认值 warmth=0.6、playfulness=0.4、directness=0.5、curiosity=0.5 | fallback 默认值与 §6 Phase 1 设计一致 | 非核心 |
| E7 | 无情绪记录时 fallback | 清空 `mood_value` 表后触发一次 prompt 构建 | 情绪值返回 0.0，prompt 中情绪段正常注入 | fallback 默认值 0.0 正确，无空指针 | 非核心 |
| E8 | 初始背景记忆自然淡化 | 写入 source='seed' 初始背景记忆后，持续对话产生真实事件摘要 | seed 背景记录停留在事件摘要早期，真实事件摘要不断累积，背景不再出现在最新上下文中 | 事件摘要按时间顺序累积，初始背景逐渐被真实记忆覆盖 | 非核心 |
...
- 功能名称：角色种子系统（v2.0 含动态情绪系统）
- 验收日期：____ 年 ____ 月 ____ 日
- 验收人员：____________
- 验收环境版本：____________
- DB 文件路径：____________
...
- [ ] E4 情绪累加下限 clamp（核心）
- [ ] E5 情绪调整值解析失败（非核心）
- [ ] E6 无种子时 fallback（非核心）
- [ ] E7 无情绪记录时 fallback（非核心）
- [ ] E8 初始背景记忆自然衰减（非核心）

### 验收结论

- 核心项通过数：____ / 15（功能 11 + 边界 4）
- 非核心项不通过数：____ / 8
- 验收结论：□ 通过   □ 附条件通过   □ 不通过
- 不通过项说明 / 补救计划：____________________________
- 验收人签字：____________
```
