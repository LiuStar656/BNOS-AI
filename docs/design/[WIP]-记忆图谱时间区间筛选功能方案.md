...
- [八、增强功能建议](#八增强功能建议)
- [九、验收方法](#九验收方法)
  - [9.1 验收环境与前置条件](#91-验收环境与前置条件)
  - [9.2 功能验收用例](#92-功能验收用例)
  - [9.3 边界与性能验收](#93-边界与性能验收)
  - [9.4 验收结论判定标准](#94-验收结论判定标准)
...
| 项 | 要求 |
|------|------|
| 数据库 | `nodes/shared/chatbot.db` 含 `long_term_memory`、`user_messages`、`diaries` 三张表，且每张表 `created_at` 字段非空记录 ≥ 5 条 |
| 时间分布 | 数据库中存在"今天"、"近7天内"、"近30天内"、"30天前"四个时段的记忆数据，便于覆盖各快捷筛选 |
| GUI | 主窗口可正常启动，知识面板 (`knowledge_panel.py`) 可正常加载图谱视图 |
| 依赖文件 | `gui/widgets/knowledge_panel.py` 已含时间筛选 UI 与 `_rebuild_graph_with_filter` 逻辑 |
| 验收工具 | 手动点击操作 + 观察图谱节点数量与状态栏文本 |

### 9.2 功能验收用例

| 编号 | 验收项 | 操作步骤 | 预期结果 | 通过标准 | 类型 |
|:----:|------|---------|---------|---------|:----:|
| F1 | 全部显示 | 进入图谱视图 → 点击 [全部] | 显示所有节点，状态栏显示"全部时间"及节点总数 | 节点数 = DB 总记录数 | 核心 |
| F2 | 今天筛选 | 点击 [今天] | 仅显示 `created_at` 在今日 0:00 之后的节点 | 节点数 ≤ DB 总数，且均为今日 | 核心 |
| F3 | 近7天筛选 | 点击 [近7天] | 仅显示最近 7 天内的节点 | 节点时间范围正确 | 核心 |
| F4 | 近30天筛选 | 点击 [近30天] | 仅显示最近 30 天内的节点 | 节点时间范围正确 | 核心 |
| F5 | 按钮互斥 | 切换不同快捷按钮 | 当前选中按钮高亮（checked），其余取消高亮 | `_update_time_btn_styles` 生效 | 非核心 |
| F6 | 自定义展开 | 点击 [自定义 ▾] | 展开日期选择器（从/到/应用/重置） | UI 正常显示 | 核心 |
| F7 | 自定义应用 | 选择起止日期 → 点击 [应用] | 仅显示该时间范围内的节点，状态栏显示范围 | 筛选结果正确 | 核心 |
| F8 | 自定义收起 | 再次点击 [自定义 ▾] | 日期选择器隐藏，恢复全部显示 | 切换正常 | 非核心 |
| F9 | 重置筛选 | 自定义范围内点击 [重置] | 日期重置为近7天~今天，图谱恢复全部 | 重置生效 | 核心 |
| F10 | 数据浏览联动 | 切换时间筛选后查看数据浏览视图 | 卡片列表同步按时间过滤 | `_rebuild_cards` 联动 | 核心 |
| F11 | 状态栏统计 | 任意筛选后查看底部状态栏 | 显示"显示: N 个节点 \| 来源: table(count) \| 筛选自: 起止日期" | 统计文本正确 | 非核心 |
| F12 | 力尺度滑块 | 拖动力尺度滑块 | 图谱力度变化，与时间筛选互不干扰 | 两个控件独立 | 非核心 |

### 9.3 边界与性能验收

| 编号 | 验收项 | 操作步骤 | 预期结果 | 通过标准 | 类型 |
|:----:|------|---------|---------|---------|:----:|
| E1 | 空数据范围 | 选择无数据的日期范围 → 应用 | 图谱清空，状态栏提示"所选时间范围内无记忆节点" | 不崩溃，有提示 | 核心 |
| E2 | 节点过少 | 选择只有 1-2 个节点的范围 | 正常显示，图谱布局不报错 | 布局稳定 | 非核心 |
| E3 | 无效日期范围 | 起始日期 > 结束日期 → 应用 | 状态栏显示错误提示，不执行筛选 | 错误提示生效 | 核心 |
| E4 | created_at 为空 | DB 中存在 `created_at` 为空的记录 | 全部模式下保留，筛选模式下排除 | `_entry_in_time_range` 容错 | 核心 |
| E5 | 时间格式异常 | DB 中存在非标准格式时间 | 多格式解析尝试，失败则按 E4 处理 | `_parse_datetime` 容错 | 非核心 |
| E6 | 防抖生效 | 连续快速点击不同筛选按钮 | 250ms 内只执行最后一次筛选 | `_filter_debounce_timer` 生效 | 核心 |
| E7 | 大数据量 | DB 记录 > 500 条时筛选 | 图谱正常加载，无明显卡顿 | 性能可接受 | 非核心 |
| E8 | 边重映射正确 | 筛选后查看图谱连线 | 仅保留两端节点均在筛选范围内的边，索引正确重映射 | `_rebuild_graph_with_filter` 正确 | 核心 |
| E9 | sim_matrix 重建 | 筛选后查看相似度矩阵 | 矩阵维度 = 筛选后节点数，对角线为 1.0 | 矩阵重建正确 | 非核心 |

### 9.4 验收结论判定标准

| 验收等级 | 判定标准 |
|------|---------|
| **通过** | 9.2/9.3 中所有"核心"项（F1-F4、F6-F7、F9-F10、E1、E3、E4、E6、E8）全部通过 |
