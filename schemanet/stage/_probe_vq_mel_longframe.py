...
"""B3: VQ 原型 + 200ms 长帧——时间尺度调整（2026-08-11）。
...
留档：runs/_probe_vq_mel_longframe_{ts}/result.json
"""
import json
import time
import wave
...
from stage._probe_sound_mel import build_net, SR, _mel_triangles
FRAME_MS = 200                  # B3: 长帧（200ms，轨迹压缩 4 倍 + 频谱平滑）
_W, _CENTERS = _mel_triangles(n_fft=2048)   # 200ms 帧需 2048 FFT
...
def mel_frames(wave, n_fft=2048):
    """声波 → 每帧 8 维 mel 向量（200ms 长帧 = 4×50ms 子帧频谱平均——时间平滑）。"""
    sub = int(SR * 50 / 1000)                  # 50ms 子帧
    win = np.hanning(sub)
    frames = []
...
def main():
    ...
    run_dir = ROOT / "runs" / f"_probe_vq_mel_longframe_{ts}"
