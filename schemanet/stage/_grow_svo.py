...
需求（用户 2026-08-10 决策链）：从"背句"（Stage 2 复述）到"造句"——按 SVO 定式
...
流程（增量，记忆不丢）：
  load_version("8.5")（词义类别，n=148604）
    ...
    → 定式跟读（训练组合 [S槽 S词 V槽 V词 O槽 O词] 共发放，R_SVO 轮）
    → 验收① 槽位归属（词→本槽位直连 ≥0.9）
    → 验收② 类别约束（动作/人称 成员→标签 ≥0.9）
    → 验收③a 定式（S槽→V槽→O槽 边存在）
...
诚实边界（2026-08-10）：
  ...
  - 造句引擎 = 显式定式走链读出（每步边都是网络真实学到的，§4.8 路径可查），
...
# "要" = 轻动词（语料"我要X"高频），进动作类但不进定式 V 集的正则约束？——
...
# V 位定式集（造句用的动作词，全部应从 ACT_MANUAL 出）
...
def svo_skeleton(ng, pats):
    """定式跟读：S槽/V槽/O槽 三槽共发放 → 槽位间定式连接（S槽→V槽→O槽）。

    与词↔槽位绑定**分离注入**（绑定用判断句两两共发放）——防定式全连接
    把 O 位约束稀释（2026-08-10 冒烟实测：O槽 与 V/S 词等强相连，造句
    top-8 名词占比 0.38 < 0.5）。分离后 O槽 出边只指向 O 词（绑定）
    + 槽位（定式）+ O 类标签（约束判断），造句聚合干净。"""
...
def make_sentence(ng, pats, n2w, s, v, vo_pairs, cat_members):
    """造句引擎（显式定式走链读出）：给定 S 词 + V 词 → O 位候选。

    三步直读，每步边都是网络真实学到的（非动力学自走）：
      step1 绑定确认：S2→S槽、V2→V槽 边存在（词认得自己的位）
      step2 定式跳：  S槽→V槽→O槽 + V槽↔动作、O槽↔食物/地点（槽位类别约束）
...
def main():
    ...
    # ── 6. 定式跟读（S槽/V槽/O槽 三槽共发放 × R_SVO 轮）──
    # 绑定已全体完成（第 4 步），定式只训槽位间连接；测试组合的"造句"由
    # 词↔槽位绑定 + 槽位↔类别约束 + 定式 泛化出（组合从未以整句共发放形式出现）
    ...
    # ── 9. 验收③a 定式（S槽→V槽→O槽 + 槽位↔类别约束 边存在）──
    ...
    print(f"[验收③a] 定式: S槽→V槽 {sk_sv:.1f} | V槽→O槽 {sk_vo:.1f} | "
          f"V槽→动作 {sk_va:.1f} | O槽→食物 {sk_of:.1f} | O槽→地点 {sk_op:.1f} "
          f"{'✅ 定式成立' if ok_skel else '❌ 定式缺失'}")
