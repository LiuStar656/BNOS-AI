...
class ChatInput(QWidget):
    ...
    def _build_ui(self):
        ...
        # 发送按钮
        self._send_btn = QPushButton(f"  发送  ")
    ...
    def eventFilter(self, obj, event):
        if obj is self._text_edit and event.type() == event.Type.KeyPress:
            key_event = event
            if key_event.key() == Qt.Key.Key_Return and not (key_event.modifiers() & Qt.ShiftModifier):
                self._do_send()
                return True
    ...
    def _add_attachment(self, attach: dict):
        # 去重：相同 path 不重复添加
        if any(a["path"] == attach["path"] for a in self._attachments):
            return
    ...
    def _do_send(self):
        ...
        if not text and not self._attachments:
            return
        self.send_requested.emit(text, list(self._attachments))
        # 清空
    ...
    def set_enabled(self, enabled: bool):
        ...
        if not enabled:
            self._text_edit.setPlaceholderText("发送中...")
        else:
            self._text_edit.setPlaceholderText("输入消息...")
